# language/syntax-acceptance-set Specification

## Purpose

M1 前端（Parser + 语法检查）的语法接受集行为契约：TileScript 模块以 Python 3.10 语法为载体（语言 API 前缀 `tis.`，源文件规范扩展名 `.tis`），定义模块顶层结构、装饰器识别、kernel 签名、设备代码语句与表达式的白名单与显式拒绝清单（错误码 `E0101`–`E0107`），以及语法拒绝报告契约（错误码/位置/类别/恢复建议四要素、收集式输出、位置排序、同位置 tiebreak、确定性）。类型检查与作用域转移、原语语义、诊断 JSON Schema 由其他 capability 承载。
